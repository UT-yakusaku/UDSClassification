...
def infer_model(model, label_data, all_data, test_indices, stride=500, window_size=2000, fs=500, device="cuda"):
    model.eval()
    all_up_states = []
...
def infer_models(models, label_data, all_data, test_indices, stride=500, window_size=2000, fs=500, device="cuda"):
    all_up_states = []
    up_coins = np.zeros(len(test_indices))
    down_coins = np.zeros(len(test_indices))
    for i, test_idx in enumerate(test_indices):
